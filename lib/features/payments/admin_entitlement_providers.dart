...
final entitlementUserDocProvider =
...
final entitlementEventsProvider =
...
final entitlementLookupProvider =
    FutureProvider.autoDispose.family<String?, String>((ref, rawInput) async {
  final input = rawInput.trim();
  if (input.isEmpty) {
    return null;
  }

  final users = ref.watch(adminEntitlementFirestoreProvider).collection('users');

  if (input.contains('@')) {
    final emailMatch = await users.where('email', isEqualTo: input).limit(1).get();
