...
final roomActivityStateProvider = StreamProvider.autoDispose
    .family<RoomActivityState, String>((ref, roomId) {
      ...
      return Stream.multi((controller) {
        ...
        final presenceSubscription = ref
            .listen<AsyncValue<List<RoomPresenceModel>>>(
              roomPresenceStreamProvider(roomId),
              (_, next) {
                final List<RoomPresenceModel> presence = next
                    .maybeWhen<List<RoomPresenceModel>>(
                      data: (value) => value,
                      orElse: () => const <RoomPresenceModel>[],
                    );
                final Map<String, bool> typing = ref
                    .read(typingStreamProvider(roomId))
                    .maybeWhen<Map<String, bool>>(
                      data: (value) => value,
