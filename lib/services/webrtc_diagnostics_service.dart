...
class WebRtcCrashlyticsDiagnostics {
  ...
  static void logConnectionError({
    ...
    try {
      ...
      crashlytics.setCustomKey('webrtc_failure_type', 'webrtc_handshake_or_ice_failure');
      ...
      crashlytics.log('[WebRTC][FAIL] Peer Count at failure: $participantCount');
      crashlytics.log('[WebRTC][FAIL] Detailed Peer ICE States: $peerIceStates');
      crashlytics.log('[WebRTC][FAIL] Error Reason: $errorDetail');

      // 3. Record non-fatal exception to trigger instant alerts
      crashlytics.recordError(
        Exception('WebRTC Drop in Room $roomId ($connectionState): $errorDetail'),
        StackTrace.current,
      ...
      Logger.info('[Crashlytics] Recorded WebRTC connection failure diagnostic successfully.');
    } catch (e, st) {
      Logger.error('[Crashlytics] Error recording diagnostic: $e', error: e, stackTrace: st);
    }
  }
}
