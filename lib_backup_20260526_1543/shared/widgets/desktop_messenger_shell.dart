...
class _DesktopMessengerShellState extends ConsumerState<DesktopMessengerShell> {
  ...
  Widget build(BuildContext context) {
    ...
    final railWidth =
        context.screenWidth >= AppBreakpoints.expanded ? 288.0 : 256.0;
...
class _MessengerSidebar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final roster = ref.watch(friendRosterProvider).valueOrNull ??
    ...
    final currentRoomId =
        ref.watch(currentUserPresenceProvider).valueOrNull?.inRoom;
    ...
    final recentConversations = conversations.where((conversation) {
      ...
      final displayName =
          conversation.getDisplayName(currentUser.id).toLowerCase();
          ...
          children: entries.map((entry) {
            ...
            return Padding(
              ...
              child: _FriendRosterRow(
                ...
                unread:
                    conversation?.hasUnreadMessages(currentUser.id) ?? false,
...
class _DesktopSocialRail extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final inRoomCount =
        roster.where((entry) => (entry.roomId ?? '').isNotEmpty).length;
    final unreadCount =
        conversations.where((c) => c.hasUnreadMessages(currentUser.id)).length;
    final pinnedCount =
        conversations.where((c) => c.isPinnedFor(currentUser.id)).length;
...
class _FriendRosterRowState extends State<_FriendRosterRow> {
  ...
  @override
  Widget build(BuildContext context) {
    final preview = widget.conversation?.lastMessagePreview ??
