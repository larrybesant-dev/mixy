...
class FollowService {
  ...
  Future<void> inviteUserToHostedRoom(String invitedUserId) async {
    ...
    final visibleHostedRooms =
        hostedRooms.where((item) => item.isVisible).toList(growable: false);
    ...
    final inviterSnapshot =
        await _firestore.collection('users').doc(inviterUserId).get();
