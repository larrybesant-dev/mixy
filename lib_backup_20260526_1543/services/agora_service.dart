...
class AgoraService extends RtcRoomService {
  ...
  @override
  String? userIdForUid(int uid) =>
      null; // Agora: caller falls back to hash lookup
  ...
  Future<void> initialize(String appId) async {
    ...
    if (_sharedInitialized) {
      ...
      final attemptTimeout =
          kIsWeb ? const Duration(seconds: 75) : const Duration(seconds: 10);
      try {
        if (kIsWeb) {
    ...
    _engine.registerEventHandler(
      RtcEngineEventHandler(
        ...
        onAudioVolumeIndication:
          ...
          final changed = nextLocalSpeaking != _localSpeaking ||
