...
final roomsSnapshotProvider = FutureProvider.autoDispose<List<RoomModel>>((
  ...
  final sections = _toSections(classified);
  // Return only active (discoverable/warm) rooms for "Live Now" feed
  return sections.primaryLive.map((item) => item.room).toList(growable: false);
});
...
final feedHealthProvider =
    Provider.autoDispose<AsyncValue<FeedHealthSnapshot>>((
  ref,
) {
  ...
  return roomsAsync.whenData((rooms) {
    ...
    final staleCount = (reasonCounts[RoomVisibilityReasonCode.warmStale] ?? 0) +
    ...
    final invalidRate =
        totalClassified == 0 ? 0.0 : invalidCount / totalClassified;
...
final liveRoomsCountProvider = FutureProvider.autoDispose<int>((ref) async {
  final rooms =
      await ref.watch(roomServiceProvider).getRoomsWithVisibility(limit: 50);
  final sections = _toSections(rooms);
...
final homeFeedSnapshotProvider =
  ...
  return AsyncValue.data(
    ref.watch(homeFeedServiceProvider).buildSnapshot(
