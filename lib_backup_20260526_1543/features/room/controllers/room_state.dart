...
class RoomStateMachine {
  ...
  static String resolveParticipantRole({
    ...
    final effectiveParticipantRole = hasAuthoritativeHost &&
            normalizedHostId != normalizedUserId &&
            participantRole == roomRoleHost
        ? ''
        : participantRole;
    final effectiveSnapshotRole = hasAuthoritativeHost &&
            normalizedHostId != normalizedUserId &&
...
class RoomState {
  ...
  bool get isRoomFullyHydrated {
    ...
    final snapshotRole = sessionSnapshotsByUser[normalizedCurrentUserId]
            ?.role
            .trim()
            .toLowerCase() ??
        '';
