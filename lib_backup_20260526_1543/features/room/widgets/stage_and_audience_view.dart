          ...
          child: Container(
            ...
            child: hasVideoStreams
                ? CameraWall(
                    ...
                    localTile: (rtcService != null)
                        ? rtcService.getLocalView()
                    ...
                    onSubscriptionPlanChanged:
                      ...
                      for (final uid in highQualityUids) {
                        rtcService.setRemoteVideoSubscription(uid,
                            subscribe: true, highQuality: true);
                      }
                      for (final uid in lowQualityUids) {
                        rtcService.setRemoteVideoSubscription(uid,
                            subscribe: true, highQuality: false);
                      }
                    },
                    ...
                    onMuteUser: (targetId, mute) =>
                        controller.muteUserToggle(targetId, mute),
                  )
                : LiveStageSpotlight(
                    roomId: roomId,
                    roomName: roomState.title,
                    displayNameById: {
                      for (final p in roomState.speakers)
                        p.userId: p.displayName ?? 'User'
                    },
                    avatarUrlById: {
                      for (final p in roomState.speakers) p.userId: p.photoUrl
                    },
                    viewerCount: roomState.memberCount,
                    primaryActionLabel:
                        roomState.speakerIds.isEmpty ? 'Take the Mic' : null,
                    onPrimaryAction: () =>
                        controller.requestMic(userId: currentUserId),
                  ),
          ),
        ),
              ...
              child: Row(
                children: [
                  Icon(Icons.people_outline,
                      size: 14, color: Color(0xFFAD9585)),
                  SizedBox(width: 8),
                  Text(
                    'AUDIENCE',
                    style: TextStyle(
