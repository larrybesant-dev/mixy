...
typedef BuildSnapshot<TSnapshot extends ConsistencySnapshot> = TSnapshot
    Function(WidgetRef ref, {required bool readOnly});
...
abstract class ConsistencyModuleContract<TSnapshot extends ConsistencySnapshot,
    TResult extends ConsistencyParityResult> {
...
ConsistencyGateDecision
  ...
  if (result.isMatch) {
    return ConsistencyGateDecision(
      ...
      emitReconcile: isPeriodicReconcile &&
  ...
  if (isPeriodicReconcile) {
    final suppress = state.lastEmittedSignature == result.signature ||
        state.lastReconcileSignature == result.signature;
    return ConsistencyGateDecision(
      emitReactiveMismatch: false,
      emitRestore: false,
