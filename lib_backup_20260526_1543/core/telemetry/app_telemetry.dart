...
class AppTelemetryState {
  ...
  AppTelemetryState copyWith({
    ...
    return AppTelemetryState(
      ...
      roomPhase:
          identical(roomPhase, _unset) ? this.roomPhase : roomPhase as String?,
      roomError:
          identical(roomError, _unset) ? this.roomError : roomError as String?,
      ...
      callError:
          identical(callError, _unset) ? this.callError : callError as String?,
...
class AppTelemetry {
  ...
  static void updateRoomState({
    ...
    if (!current.presenceMismatch && next.presenceMismatch) {
      ...
      logAction(
        ...
        domain: 'presence',
        action:
            isSuppressed ? 'presence_mismatch_suppressed' : 'presence_mismatch',
        message: isSuppressed
    ...
    for (final entry in event.metadata.entries) {
      ...
      if (value is bool) {
        ...
        if (asString.isNotEmpty) {
          params[key] =
              asString.length <= 100 ? asString : asString.substring(0, 100);
        }
      }
    }
  ...
  static RoomHealthSnapshot _buildRoomHealthSnapshot(AppTelemetryState state) {
    ...
    final inRecoveryWindow = state.roomPhase == 'joining' ||
    ...
    final roomMismatch = state.roomPhase == 'joined' &&
        state.roomId != null &&
  ...
  static void _logRoomHealthTransition({
    ...
    for (final alert in next.alerts) {
      ...
      logAction(
        level:
            alert.severity == RoomHealthSeverity.critical ? 'error' : 'warning',
        domain: 'room',
        action: 'health_alert',
