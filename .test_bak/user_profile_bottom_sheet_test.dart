  ...
  group('UserProfileBottomSheet Widget and State Binding Tests', () {
    ...
    testWidgets('1. Loading State - Verifies that the pulsing glassmorphic shimmer layout is shown', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            // Use Completer.future to return an unresolved Future that keeps the provider in a clean loading state
            userProfileFutureProvider(testUserId).overrideWith((ref) => Completer<UserProfile>().future),
            userPresenceStreamProvider(testUserId).overrideWith((ref) => const Stream.empty()),
          ],
    ...
    testWidgets('2. Data State - Verifies dynamic profile data & interactive Follow/Unfollow toggle', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            // Standard Riverpod .overrideWith blocks for dynamic test state
            userProfileFutureProvider(testUserId).overrideWith((ref) => mockProfile),
            userPresenceStreamProvider(testUserId).overrideWith((ref) => Stream.value(mockPresence)),
          ],
          child: MaterialApp(
            home: Scaffold(
      ...
      // Let any microtasks/stream emissions settle first
      await tester.pump();
    ...
    testWidgets('3. Error State - Verifies user friendly recovery dialog displays', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            userProfileFutureProvider(testUserId).overrideWith(
              (ref) => Future.error(Exception('Firestore security rule check failed'))),
            userPresenceStreamProvider(testUserId).overrideWith(
              (ref) => Stream.value(const UserPresence(isOnline: false))),
          ],
          child: MaterialApp(
            home: Scaffold(
              body: UserProfileBottomSheet(
      ...
      // Allow the Future.error to complete and trigger the widget re-render
      await tester.pump();
