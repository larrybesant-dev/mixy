  ...
  try {
    ...
    try {
      await page.waitForFunction(
        ...
        { timeout: APP_READY_TIMEOUT_MS, polling: 100 },
      );
    } catch (error) {
      failureReason = 'app_contract_failure';
      const contractOnTimeout = await page.evaluate((key) => {
        return window.sessionStorage.getItem(key);
      }, APP_READY_CONTRACT_KEY).catch(() => 'failed_to_eval');
      const timelineLogs = await page.evaluate(() => {
        return window.sessionStorage.getItem('startupLogs');
      }).catch(() => 'failed_to_eval');
      failureDetail = `${toErrorText(error)}\n[DIAGNOSTIC] contract on timeout: ${contractOnTimeout}\n[DIAGNOSTIC] timeline logs:\n${timelineLogs}`;
      throw error;
    }
