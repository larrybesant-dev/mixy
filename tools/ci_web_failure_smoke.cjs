...
async function readAppReadyContract(page) {
  ...
  }, APP_READY_CONTRACT_KEY).catch(() => null);
}
...
async function runOfflineScenario(browser) {
  ...
  // Load the local index.html directly from build/web/index.html to simulate
  // the page being loaded or cached offline, while other network requests fail.
  const indexPath = path.join(__dirname, '../build/web/index.html');
  let indexHtml = '';
  try {
    indexHtml = await fs.readFile(indexPath, 'utf8');
  } catch (_) {
    indexHtml = '<html><body><div id="boot-shell"><p id="boot-msg">Unable to load app runtime</p></div></body></html>';
  }

  // Intercept the main page request to serve the index.html content even when offline
